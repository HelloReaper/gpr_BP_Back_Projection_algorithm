...
if __name__== "__main__" :
    ...
    # 计算成像区域每个点A在各道数据上的散射响应幅值并叠加，保存在一个二维数组中
    for page in range(len(xr)):
        field_data = field[:,page]
        double_time = []
        filepath = '../Data/DoubleTimes_%d.txt'%(page)
        dt_file = open(filepath,'r')
        for dt in dt_file:
            dtime = dt.strip("\n").split()
        ...
        for row in range(len(xA)):
            ...
            for col in range(len(zA)):
                # 将成像区域每个点在各道回波数据上的散射响应幅值进行叠加
                amplitudes[row,col] +=  abs(newarr[col])

    xx,yy = np.meshgrid(xA,zA,indexing='ij')
    print("shape of original dataset:\n\txx:{},yy:{},z:{}".format(xx.shape,yy.shape,amplitudes.shape))
    fig = plt.figure(figsize=(10, 10), facecolor='white') #创建图片
    sub = fig.add_subplot(111, projection='3d')# 添加子图，
    surf = sub.plot_surface(xx, yy, amplitudes, cmap=plt.cm.brg) #绘制曲面,cmap=plt.cm.brg并设置颜色cmap
    ...
    amplitudes_file = '../Data/amplitudes_adds.txt'
    amp_fid = open(amplitudes_file,'w+')
    for r in range(rows):
        for c in range(cols):
            amp_fid.write(str(amplitudes[r,c])+'\t')
        amp_fid.write('\n')
    amp_fid.close()